          ...
          {enrolledCourses.map((course, index) =>(
            ...
            <tr key={index} className='text-gray-700'>
                ...
                <td className='px-4 py-3 max-sm:text-right'>
                  <button onClick={()=> navigate('/player/' + course._id)}  className='px-4 py-3 max-sm:px-5 sm:py-2 bg-blue-600 max-sm:text-xs text-white rounded'>{progressArray[index] && progressArray[index].lectureCompleted / progressArray[index].totalLectures === 1 ? 'Completed': 'Ongoing'}</button>
                </td>
            </tr>
          ))}
        </tbody>
