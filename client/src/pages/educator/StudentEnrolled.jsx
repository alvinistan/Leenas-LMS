...
const StudentEnrolled = () => {
  ...
  const fetchEnrolledStudens = async () => {
          ...
          <tbody className='text-sm text-gray-500'>
            {enrolledStudents.map((item, index) => (
              <tr key={item.student._id} className='border-b border-gray-500/20' >
                  ...
                  <td className='md:px-4 px-2 py-3 flex items-centre space-x-3'>
                    <img className='h-9 w-9 rounded-full' src={item.student.imageUrl} alt=""/>
