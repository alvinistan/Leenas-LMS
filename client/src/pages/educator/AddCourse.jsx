...
const AddCourse = () => {
  ...
  return (
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pb-0">
      ...
      {/* Adding Course Chapters & Lectures */}
      <div>
              ...
              <div className="p-4">
                {chapter.chapterContent.map((lecture, lectureIndex) => (
                  <div key={lectureIndex} className="flex justify-between items-center mb-2">
                    <span>
                      {lectureIndex + 1} {lecture.lectureTitle} -{lecture.lectureDuration} mins - <a href={lecture.lectureUrl} target="blank" className="text-blue-500">Link</a>
                      - {lecture.isPreviewFree ? 'Free Priview' : 'paid'}
                      <img src={assets.cross_icon} alt="" className="cursor-pointer" onClick={()=>handleLecture('remove',chapter.chapterId,lectureIndex)}/>
                    </span>
                  </div>
                ))}
                <div className="inline-flex bg-gray-100 p-2 rounded mt-2 cursor-pointer"onClick={()=>handleLecture('add',chapter.chapterId)}>
                  + Add Lecture
                </div>
              </div>  
            )}
        ...
        {showPopup && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-00/40 backdrop-blur-sm z-50">
